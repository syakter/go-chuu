...
func TestParser_Parse(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "chart command with custom size NxN",
			input: "!chart user1 7d 5x5",
			expected: &types.Command{
				Type:      types.CommandChart,
				User:      "user1",
				Period:    "7d",
				ChartSize: 5,
				RawInput:  "!chart user1 7d 5x5",
			},
			wantErr: false,
		},
		{
			name:  "chart command with custom size N",
			input: "!chart user1 7d 4",
			expected: &types.Command{
				Type:      types.CommandChart,
				User:      "user1",
				Period:    "7d",
				ChartSize: 4,
				RawInput:  "!chart user1 7d 4",
			},
			wantErr: false,
		},
		{
			name:    "chart command with invalid size",
			input:   "!chart user1 7d 11x11",
			wantErr: true,
		},
		{
